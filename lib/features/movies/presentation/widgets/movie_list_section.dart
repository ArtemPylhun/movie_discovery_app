...
class MovieListSection extends ConsumerWidget {
  ...
  final StateNotifierProvider<MovieListNotifier, MovieListState> provider;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      ...
      child: state.when(
        ...
        loadingMore: (movies) => _buildMovieGrid(context, ref, movies, true),
        loaded: (movies, hasReachedMax) => movies.isEmpty
            ...
            : _buildMovieGrid(context, ref, movies, false, hasReachedMax),
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (scrollInfo) {
        if (!hasReachedMax &&
          ...
          ref.read(provider.notifier).loadMoreMovies();
        }
        return false;
      },
...
// Extension to add when method to MovieListState
extension MovieListStateExt on MovieListState {
  T when<T>({
